...
async def buy(current_user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def disable(current_user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
