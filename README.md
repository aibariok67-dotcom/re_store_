...
> ВАЖНО! Сервер использует sleep loading - пока запросов нет, сервер засыпает. Для того чтобы он заработал, нужно подождать максимум минуту.
...
- На странице игры (и через API) можно запросить краткий разбор пользовательских отзывов через внешний LLM в формате OpenAI **Chat Completions** (совместимые прокси тоже подходят, если задать `AI_REVIEWS_API_URL`).
- **Эндпоинт:** `POST /ai/games/{game_id}/reviews-summary` — нужен JWT (как у остальных защищённых методов). Лимит запросов: **30/час** на IP (slowapi).
- В модель уходят только **оценка и текст** отзывов по этой игре (без имён и прочих полей), объём ограничен настройками `AI_REVIEWS_MAX_FETCH`, `AI_REVIEWS_MAX_CONTEXT_CHARS`, `AI_REVIEWS_PER_REVIEW_MAX_CHARS`.
- Если отзывов меньше порога **`AI_REVIEWS_MIN_COUNT`** (по умолчанию 3), API вернёт **400** с понятным текстом. Без **`OPENAI_API_KEY`** — **503** с подсказкой настроить ключ.
- Опциональные переменные см. в **`.env.example`** (`OPENAI_API_KEY`, `AI_REVIEWS_MODEL`, таймаут и т.д.).
...
2. Обязательно задайте: `DATABASE_URL`, `SECRET_KEY`, `CLOUDINARY_CLOUD_NAME`, `CLOUDINARY_API_KEY`, `CLOUDINARY_API_SECRET` (ключи — в [Cloudinary Dashboard](https://cloudinary.com/)). Для AI-сводки отзывов добавьте `OPENAI_API_KEY` (см. блок в `.env.example`); без ключа эндпоинт AI просто ответит ошибкой конфигурации.
